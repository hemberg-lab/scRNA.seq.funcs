...
run_pca <- function(x, retx = TRUE, center = TRUE, scale = TRUE) {
    ...
    testit::assert("Variance explained is calculated correctly.",
...
plot_pca <- function(x, pcx = 1, pcy = 2, explained = NULL, metadata = NULL,
    ...
    if (!is.null(metadata)) {
        testit::assert("PC and metadata have same number of rows.",
    ...
    if (!is.null(explained)) {
        testit::assert("Number of PCs differs between x and explained.",
...
make_pca <- function(x) {
    res <- run_pca(x)
    plot_pca(res$PCs, explained = res$explained,
             metadata = blischak.ann, color = "replicate",
             shape = "individual")
}
